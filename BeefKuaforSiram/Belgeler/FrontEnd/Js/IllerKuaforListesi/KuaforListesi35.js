...
app.controller("kuafordetaybilgiler", function ($scope, $http) {
    var pathArray = window.location.pathname.split('/');
    var secondLevelLocation = pathArray[3];
    $http.get("http://localhost:52747/api/Kuaforler/DetayBilgilerIcerik/" + secondLevelLocation).then(function (response) {
...
app.controller("kullaniciprofilsayfasi", function ($scope, $sce) {
    ...
    $scope.kmesajkutusu = function () {
        $scope.hiddenmesajlarim = !$scope.hiddenmesajlarim;
...
app.controller("kullaniciMesajlarKullaniciKutusu", function ($scope, $http) {//----------
    var pathArray = window.location.pathname.split('/');
    var secondLevelLocation = pathArray[3];
    $http.get("http://localhost:52747/api/KuaforMesaj/GETKullaniciMesajKuforler/" + secondLevelLocation).then(function (response) {
        $scope.kullaniciCuzdanKntrl = response.data;
    });
